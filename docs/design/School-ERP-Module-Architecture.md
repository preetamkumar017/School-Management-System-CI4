...
|---|---|---|---|---|
| Administration | `App\Modules\Administration` | User (`ENT-SYS-001`), Role (`ENT-SYS-002`), AuditLog (`ENT-SYS-004`), Configuration (`ENT-SYS-005`), Document (`ENT-SYS-006`), ApprovalRequest (`ENT-SYS-007`) | **`User`/`Role`/`AuditLog`/`Configuration`/`Document` designed** — see `docs/design/administration/`; `ApprovalRequest` not yet designed, deferred per `School-ERP-Development-Roadmap.md` Stage 1 | ADR-002 (confirms no `GuardianService` belongs here), ADR-011 (`Configuration`), ADR-012 (`Document`) |
